...
if triangles
	...
	H=patch(surf2patch(x,y,zeros(size(x)),im2double(c),'triangles'),'edgecolor','black','facecolor','flat');
else
    %H=surface(x,y,zeros(size(x)),c,'EdgeColor','none','FaceColor','texturemap');
	H=surface(x,y,zeros(size(x)),c,'EdgeColor','black','FaceColor','texturemap');
end
