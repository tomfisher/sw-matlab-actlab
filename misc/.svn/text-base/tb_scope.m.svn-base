    ...
    function tb_connect(h,eventdata)
        ...
        if TCP_SERVER >= 0
            ...
            if B_SPETCTRUM
                ...
                PLOT_HANDLES = bar(data(end,B_SPETCTRUM_START_IDX:end));
                colormap(YELLOW);
                shandle = gca;
                set(shandle, 'XGrid','on','YGrid','on', ...
                    'Color','k','XColor',YELLOW,'YColor',YELLOW,'YLim',[0,255]);
            else
            ...
            try
                while CONNECTED
                    ...
                    if B_SPETCTRUM && nData > 0 && nDim == DIM && ~bFrozen
                        set(PLOT_HANDLES,'YData',data(end,B_SPETCTRUM_START_IDX:end));
                    elseif nData > 0 && nDim == DIM
                            DATA_BUFFER = circshift(DATA_BUFFER,[-nData 0]);
                            DATA_BUFFER(end-nData+1:end,:) = data;
                            if ~bFrozen
                                for d = 1:DIM 
