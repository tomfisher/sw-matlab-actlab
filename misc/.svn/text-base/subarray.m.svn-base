...
%   >> subarray(M,1) retrieves the first line of matrix M
%   >> subarray(M,[3 2],2) retrieves the 3rd and 2nd columns of M (in that order) 
...
%   >> subarray(M,[-1 -2],-Inf) retrieves all but the second-to-last and next-to-last
%                             elements of M(:)
...
if any(isinf(dim))
    if length(dim)>1
        error('In specifying dimensions, dim=+Inf or dim-Inf requires to be used alone.')
    end
...
nd=ndims(X);
%define the structre to be passed to SUBSERF function
S.type='()';
S.subs=cell(nd,1);
