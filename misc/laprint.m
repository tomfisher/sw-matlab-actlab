...
if head
  fprintf(fid,[ '%% This file is generated by the MATLAB m-file' ...
       ' laprint.m (Version OAM, WARNING: Does not behave as original version). It can be included\n']);
...
if numbertext>0,
  ...
  for i=1:ntp
    if length(newstr{i})
      ...
      % OAM REVISIT: added hard-coded font size
	  oam_fontsize = oam_getcmd(get(htext(i), 'tag'), 'laprint_fs'); 
	  if isempty(oam_fontsize), oam_fontsize = 1.0; end;
	  
      fprintf(fid,[ '\\psfrag{' newstr{i} '}[' alig '][' alig '][' num2str(oam_fontsize,'%.1f') ']{' ...
  ...
  for ixyz=1:3
    ...
    if sum(nlabel) > 0
      ...
      for i=1:na
          % OAM REVISIT: added hard-coded font size
		  oam_fontsize = oam_getcmd(get(htext(i), 'tag'), 'laprint_fs');
		  if isempty(oam_fontsize), oam_fontsize = 0.8; end;
		  
        poss = ['[' xyzalign(i,ixyz) '][' xyzalign(i,ixyz) '][' num2str(oam_fontsize,'%.1f') ']']; 
        if nlabel(i)
...
% figure
% OAM REVISIT: Do not insert figure here, commented out
fprintf(fid,'%%\n');
fprintf(fid,'%% Figure:\n');
if iscaption
...
function val = oam_getcmd(cmdstring, cmd)
val = [];
cmdstr = str2cell(cmdstring,',');
pos = strmatch(cmd, cmdstr);
if isempty(pos), return; end;
cmdstr = cmdstr{pos}; cmdstr(cmdstr==' ') = [];
pos = strfind(cmdstr,'=');
if isempty(pos), return; end;
cmdstr(1:pos+1) = []; 
val = str2double(cmdstr);
