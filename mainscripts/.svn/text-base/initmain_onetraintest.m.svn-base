% initmain_onetraintest
% 
% Perform only ONE train-test iteration, using initial training phase and subsequent experiment.
% Changes:  CVSectionBounds, labellist_load
% 
% optional parameters:
...
% if size(trainsection,1) > 1
%     fprintf('\n%s: WARNING: Found more than one training section (total: %u).', mfilename, size(trainsection,1)); 
% end;
...
% add gaps as testsections
CVSectionBounds = [ trainsection(:,1:2); ...
    segment_findgaps(trainsection(:,1:2), 'Maxsize', max(labellist_load(:,2)), 'IncludeBoundaries', true) ];

% adapt labellist
% trainlabelmarks = vec2onehot(segment_findincluded(trainsection, labellist), size(labellist,1));
