...
for i=1:length(seglist)
    ...
    for startpoint = seglist(i,1):50:seglist(i,2)
        
        endpoint = startpoint + 100 -1;
        if endpoint < seglist(i,2)  % not the last window
            index =index +1;
            seglist_CONT(index,:) = [startpoint endpoint (endpoint-startpoint+1) seglist(i,4) index 1];
        end
    end
...
if ( size(seglist_CONT,1) ~= size(fmatrix,1) ), 	error('Segment list and feature matrix do not match.'); end;
...
if ( ~all(findn(1:length(thisTargetClasses), unique(seglist_CONT(:,4)))) ), error('Some classes have no observations.');  end;
...
subjectidlist = zeros(size(seglist_CONT,1), 1);
...
% 		[dummy partlabelidx] = repos_findlabelsforpart(seglist, subjectpartidx(partno), partoffsets);
        		[dummy partlabelidx] = repos_findlabelsforpart(seglist_CONT, subjectpartidx(partno), partoffsets);

		subjectidlist(partlabelidx) = s;
	end;
end;
% seglist(:,end+1) = subjectidlist; % append subjectidlist to seglist
seglist_CONT(:,end+1) = subjectidlist; % append subjectidlist to seglist

% fprintf('\n%s:   size(seglist)=%s', mfilename, mat2str(size(seglist)));
fprintf('\n%s:   size(seglist)=%s', mfilename, mat2str(size(seglist_CONT)));
...
%     allIndices{class} = col(find( (seglist(:,4) == class) & (seglist(:,6) >=LabelConfThres) ));
    allIndices{class} = col(find( (seglist_CONT(:,4) == class) & (seglist_CONT(:,6) >=LabelConfThres) ));
...
% sellabels = seglist(:,6)>=LabelConfThres;
sellabels = seglist_CONT(:,6)>=LabelConfThres;

% fprintf('\n%s: Training: %u of %u labels selected.', mfilename, sum(sellabels), size(seglist,1));
fprintf('\n%s: Training: %u of %u labels selected.', mfilename, sum(sellabels), size(seglist_CONT,1));
...
%         [trainIndices testIndices] = classgroupcv(seglist, subjectidlist, 'ClassCol', 4, 'ReturnIdx', true);
                [trainIndices testIndices] = classgroupcv(seglist_CONT, subjectidlist, 'ClassCol', 4, 'ReturnIdx', true);
...
%         datasliceidlist = zeros(size(seglist,1),1);
                datasliceidlist = zeros(size(seglist_CONT,1),1);

%         for i = 1:size(dataslices,1),  datasliceidlist = datasliceidlist + segment_markincluded(dataslices(i,:), seglist)*i;   end;
        for i = 1:size(dataslices,1),  datasliceidlist = datasliceidlist + segment_markincluded(dataslices(i,:), seglist_CONT)*i;   end;
% [trainIndices testIndices] = classgroupcv(seglist, datasliceidlist, 'ClassCol', 4, 'ReturnIdx', true);        
        [trainIndices testIndices] = classgroupcv(seglist_CONT, datasliceidlist, 'ClassCol', 4, 'ReturnIdx', true);        
...
%             testIndices{1}{class} = col(find(  (seglist(:,4) == class) & (seglist(:,6) >= PRLabelConfThres) & (seglist(:,6) < LabelConfThres)));            
            allIndices{class} = col(find(seglist_CONT(:,4) == class));
            trainIndices{1}{class} = col(find(  (seglist_CONT(:,4) == class) & (seglist_CONT(:,6) >= LabelConfThres) ));
            testIndices{1}{class} = col(find(  (seglist_CONT(:,4) == class) & (seglist_CONT(:,6) >= PRLabelConfThres) & (seglist_CONT(:,6) < LabelConfThres)));            
        end;
...
% 		mat2str(cellfun('size', segments2classlabels(length(unique(seglist(:,4))), seglist),1)));
	fprintf('\n%s:   Total: %s', mfilename, ...
		mat2str(cellfun('size', segments2classlabels(length(unique(seglist_CONT(:,4))), seglist_CONT),1)));
...
% 	testlabels = seglist(cell2mat(testIndices{cviters}),:);
	trainlabels = seglist_CONT(cell2mat(trainIndices{cviters}),:);
	testlabels = seglist_CONT(cell2mat(testIndices{cviters}),:);
...
% 	gtseglist = labellist_load(segment_findequals(labellist_load, seglist),:);
	gtseglist = labellist_load(segment_findequals(labellist_load, seglist_CONT),:);
...
%         thisclass(class) = seglist(find(gtseglist(:,4) == realclass(class),1), 4); 
        thisclass(class) = seglist_CONT(find(gtseglist(:,4) == realclass(class),1), 4); 
...
%         'StartTime', 'SaveTime');
    save(isoclass_filename, ...
        ...
        'prmetrics', 'prmetricslist', 'scmetrics', 'cmetrics', 'cmetricslist', 'emetrics', ...
        'thisTargetClasses', 'Classlist', 'FeatureString', 'Filtered_FeatureString', 'fm_FeatureString', 'FSelFeatureCount', ...
        'seglist_CONT', 'MergeClassSpec', 'Partlist', 'DSSet', 'CVFolds', ...
        'trainIndices', 'testIndices', ...
        'StartTime', 'SaveTime');
    fprintf('done.\n');
end;
